...
# Ensure project root is importable (parent of component/)
...
class TestHistoryManager:
    ...
    def test_create_and_get_run(self, tmp_dir):
        """create_run returns a valid ID; get_run retrieves it."""
        from StellarOrionEngineMach5Up import HistoryManager
        ...
        assert run["name"] == "test_run"  # name column (dict access)
    ...
    def test_update_run_progress(self, tmp_dir):
        ...
        run = hm.get_run(run_id)
        assert run["current_sample"] == 5  # current_sample column (dict access)
...
class TestApiCalculateShieldMassAnalytical:
    ...
    Axiom: Sphere-cone geometry — nose cap is spherical, flank is conical.
    ...
    def test_larger_diameter_increases_mass(self):
        ...
        assert large["shield_mass_kg"] > small["shield_mass_kg"]
