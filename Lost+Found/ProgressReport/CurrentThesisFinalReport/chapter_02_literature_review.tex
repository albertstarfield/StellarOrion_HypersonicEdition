...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.45\textwidth}
        ...
        {\small \textbf{(a)} First Type Description}
        \label{fig:tps_type0}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.45\textwidth}
        ...
        {\small \textbf{(b)} Second Type Description}
        \label{fig:tps_type1}
    \end{minipage}
    ...
    \begin{minipage}[t]{0.45\textwidth}
        ...
        {\small \textbf{(c)} Third Type Description}
        \label{fig:tps_type2}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.45\textwidth}
        ...
        {\small \textbf{(d)} Fourth Type Description}
        \label{fig:tps_type3}
    \end{minipage}
...
\noindent In this study, a PINN is implemented using the DeepXDE library \cite{lu2021deepxde} with a fully connected feedforward architecture consisting of 2 input features (radial and axial coordinates in the axisymmetric plane), 5 hidden layers of 128 neurons each with $\tanh$ activation, and 5 output features (velocity components, temperature, and pressure). The network is trained in two stages: first, pre-training on synthetic boundary condition data to establish the solution topology, and second, fine-tuning using sampled DSMC field data to correct statistical noise and fill spatial gaps. This two-stage protocol ensures the network satisfies the governing partial differential equations while remaining anchored to the physics captured by the particle simulation. The detailed training protocol and validation results are presented in Chapter~3.
...
\noindent These configurations represent optimized material stackups that account for the coupled thermal--structural response of the inflated aeroshell. The Rapisarda model is used as the baseline for all simulations in this study, calibrated against the IRVE-3 flight data (peak heat flux 14.36~W/cm$^2$ model vs.\ 13.8~W/cm$^2$ flight; total heat load 195.06~J/cm$^2$ model vs.\ 188~J/cm$^2$ flight).
...
\begin{figure}[htbp]
    ...
    \caption{Knudsen regime map as a function of altitude for the 3.0~m HIAD. The Knudsen number $Kn = \lambda / L$ determines the appropriate flow solver: continuum Navier--Stokes ($Kn < 0.01$), transitional DSMC ($0.01 < Kn < 10$), or free molecular ($Kn > 10$). At the IRVE-3 re-entry reference altitude of 52~km, the flow is in the transitional regime ($Kn \approx 0.03$), requiring DSMC simulation.}
...
\begin{enumerate}
    ...
    \item \textbf{Machine Learning for CFD:} Physics-Informed Neural Networks embed governing PDEs into the loss function, enabling mesh-free solution refinement and gap-filling from sparse DSMC data (Section 2.14).
    ...
    \item \textbf{Optimization Theory:} Multidisciplinary design optimization (MDO) combined with Latin Hypercube sampling, surrogate modelling (MoP), and genetic algorithm search provides the theoretical framework for navigating the coupled aerodynamic--thermal--structural design space (Section 2.16).
\end{enumerate}
