...
Despite the thermal constraint violation, the GA identified the IRVE-3 baseline configuration (Run~27, Sample~8) as the optimal design point: $D = 3.0$\,m, $\theta_c = 45°$, $N = 6$, $R_N = 0.5$\,m. This configuration achieves $\beta = 29.27$\,kg/m$^2$ and $n_G = 21.37g$, which are within acceptable ranges for cargo reentry. The convergence of the optimizer toward the IRVE-3 geometry validates the physical consistency of the framework, as the IRVE-3 design represents a flight-tested configuration optimized through decades of NASA research~\cite{lau2013irve3}.
...
\begin{table}[H]
    ...
    \begin{tabular}{p{3.5cm} p{5cm} p{5cm}}
        ...
        Thermal Analysis & 1D F-TPS with contact conductance & Sutton-Graves correlation + DSMC surface computes \\
