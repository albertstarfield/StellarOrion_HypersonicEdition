...
Bird's Direct Simulation Monte Carlo method~\cite{bird1994dsmc} provides a stochastic solution to Equation~\eqref{eq:boltzmann}. As the number of simulated particles $N \to \infty$, the DSMC solution converges to the exact Boltzmann solution (Axiom~A6). The DSMC algorithm proceeds in three steps per time step:
\begin{enumerate}
...
For rapid engineering estimates, the stagnation-point heat flux is approximated by~\cite{sutton1971approximate}:
\begin{equation}
...
The Physics-Informed Neural Network enforces the Navier--Stokes residuals as a soft constraint in the loss function~\cite{raissi2019physics}:
\begin{equation}
...
\multirow{3}{*}{\textbf{Flight Conditions}} & $V_\infty$ & 2700\,m/s \\
 & $T_\infty$ & 270.0\,K \\
 & $n_\infty$ & $3.47 \times 10^{21}$\,/m$^3$ \\ \midrule
...
\multirow{6}{*}{\textbf{PINN (DeepXDE)}} & Architecture & FNN [2] + [128] $\times$ 5 + [5] \\
 & Activation & tanh \\
...
\multirow{4}{*}{\textbf{MoP (PyTorch)}} & Architecture & MLP [n $\to$ 128 $\to$ 128 $\to$ 8] \\
 & Activation & ReLU \\
 & Learning rate & 0.005 \\
...
\begin{itemize}
    ...
    \item \textbf{Number Density ($n$):} $3.47 \times 10^{21}$\,m$^{-3}$ (derived from atmospheric density $\rho \approx 1.67 \times 10^{-4}$\,kg/m$^3$ via $n = \rho N_A / M_{air}$)
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|}
        ...
        Input dimension & 2 $(x, y)$ \\ \hline
        Hidden layers & 5 \\ \hline
        Neurons per layer & 128 \\ \hline
        Activation function & $\tanh$ \\ \hline
        Weight initialization & Glorot uniform \\ \hline
        Output dimension & 5 $(\rho, u, v, T, p)$ \\ \hline
        Total trainable parameters & $2 \times 128 + 4 \times 128^2 + 5 \times 128 + 5 \approx 67{,}000$ \\ \hline
    \end{tabular}
\end{table}
...
where $\mathbf{x} = (x, y)$ is the spatial coordinate and $\hat{\mathbf{y}} = (\hat{\rho}, \hat{u}, \hat{v}, \hat{T}, \hat{p})$ is the predicted flow field vector. All outputs are in normalized (dimensionless) form; physical units are recovered by multiplication with the scaling factors.
...
\begin{equation}
    \bar{\rho} = \frac{\rho}{\rho_{ref}}, \quad \bar{u} = \frac{u}{u_{ref}}, \quad \bar{v} = \frac{v}{u_{ref}}, \quad \bar{T} = \frac{T}{T_{ref}}, \quad \bar{p} = \frac{p}{p_{ref}}, \quad \bar{\mathbf{x}} = \frac{\mathbf{x}}{L_{ref}}
\end{equation}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|l|}
        ...
        PDE loss weights & $[0, 0, 0, 0, 0]$ & $[10^{-4}] \times 5$ \\ \hline
        Data loss weights & $[1.0] \times 5$ & $[1.0] \times 5$ \\ \hline
        ...
        Initialization & Glorot uniform & (continued from Stage 1) \\ \hline
    \end{tabular}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|}
        ...
        Input dimension & 4 ($x, y, z, t$ or spatial coordinates) \\ \hline
        Output dimension & 5 ($\rho, u, v, w, T$) \\ \hline
        Hidden layers & 5 \\ \hline
        Neurons per layer & 128 \\ \hline
        Activation function & $\tanh$ \\ \hline
        Weight initialization & Glorot uniform \\ \hline
        Total parameters & $\mathcal{O}(10^5)$ \\ \hline
    \end{tabular}
\end{table}
