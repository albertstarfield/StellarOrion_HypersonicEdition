...
\textit{These gaps will be further elaborated in Chapter IV and Chapter V.}
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Numerical Framework:} The gas dynamics are governed by the Boltzmann equation, solved using the Direct Simulation Monte Carlo (DSMC) method as implemented in the SPARTA (Sandia Parallel Algorithm for Real-Time Application) solver~\cite{Plimpton2014}. DSMC is valid for rarefied and transitional flows where the continuum assumption fails ($Kn > 0.01$).
...
The simulation results are validated against IRVE-3 flight data~\cite{Rapisarda2023} using the Sutton-Graves correlation. Key validation metrics include peak heat flux (13.8 W/cm$^2$ flight vs.~14.36 W/cm$^2$ model), total heat load (188 J/cm$^2$ flight vs.~195.06 J/cm$^2$ model), and peak deceleration (19.7$g$ flight vs.~20.2$g$ model). The final optimized design undergoes a high-fidelity SPARTA validation run \textit{without} PINN refinement to eliminate neural network bias.
...
\textbf{CHAPTER IV} This chapter presents the preliminary hypotheses and predicted aerothermal phenomena based on the literature study and DSMC simulation results. It synthesizes existing research and simulation data to establish performance expectations, including Knudsen number distributions, shock structure characteristics, and heating augmentation in the scalloped toroidal valleys.
