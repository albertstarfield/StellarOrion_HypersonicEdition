...
package body StellarOrion_Project is
   ...
   begin
      ...
      Put_Line ("[TEST] Running self-test ...");
      New_Line;

      --  Test 1: Mean free path
      T1 := Mean_Free_Path (1.0e23, MOL_DIAM);
      Put_Line ("[TEST] MFP(n=1e23, d=3.7e-10) = " & Float'Image (T1) & " m");
      Put_Line ("[TEST]   Expected ~ 5.2e-3 m");
      New_Line;

      --  Test 2: Knudsen number
      T2 := Knudsen_Number (T1, 3.0);
      Put_Line ("[TEST] Kn(MFP, D=3m) = " & Float'Image (T2));
      Put_Line ("[TEST]   Expected ~ 1.7e-3 (continuum-transition)");
      New_Line;

      --  Test 3: Sutton-Graves heat flux
      T3 := Sutton_Graves_Heat (6.9674e-4, 0.55, 2700.0);
      Put_Line ("[TEST] q_stag(Mach10, 52km) = " & Float'Image (T3) & " W/m^2");
      Put_Line ("[TEST]   Expected ~ 140,000 W/m^2 (14 W/cm^2)");
      New_Line;

      --  Test 4: Geometry validation
      Geo := (others => <>);  -- defaults (IRVE-3)
      if Validate_Geometry (Geo) then
         Put_Line ("[TEST] IRVE-3 geometry: VALID");
      else
         Put_Line ("[TEST] IRVE-3 geometry: INVALID (unexpected!)");
      end if;
      New_Line;
      ...
      Put_Line ("[TEST] Metrics pipeline OK");
      ...
      Put_Line ("[TEST] All self-tests PASSED.");
   ...
   begin
      ...
      Put_Line ("  Velocity : " & Float'Image (Flight.Velocity_Ms) & " m/s");
      Put_Line ("  Diameter : " & Float'Image (Geo.Diameter_M) & " m");
      Put_Line ("  Nose     : " & Float'Image (Geo.Nose_Radius_M) & " m");
      Put_Line ("  TPS      : " & TPS.Name);
      New_Line;

      --  Analytical heat flux via Sutton-Graves
      Results.Heat_Flux_Wm2 :=
        Sutton_Graves_Heat (Flight.Density_Kgm3,
                            Geo.Nose_Radius_M,
                            Flight.Velocity_Ms);
      ...
      --  Cd = 1.5 (Rapisarda 2023, Table 4.1 — blunt HIAD at Mach 10)
      ...
      --  Calculate full flight metrics
      Calculate_Flight_Metrics (Results, Flight, Geo, TPS, Metrics);

      --  Print comparison table
      Put_Line ("  -------------------------------------------------------");
      Put_Line ("  Parameter          | Analytical  | Flight Target");
      Put_Line ("  -------------------------------------------------------");
      Put_Line ("  Heat flux (W/cm^2) | " &
                Float'Image (Metrics.Stag_Heat_Flux_Wcm2) & "   | " &
                Float'Image (Target_Heat_Flux));
      ...
      Put_Line ("  Beta (kg/m^2)      | " &
                ...
                Float'Image (Target_Beta));
      Put_Line ("  Surface temp (K)   | " &
                Float'Image (Metrics.Surface_Temp_K));
      Put_Line ("  Backface temp (K)  | " &
                Float'Image (Metrics.Backface_Temp_K));
      Put_Line ("  Survivable         | " &
                Boolean'Image (Metrics.Survivable));
      Put_Line ("  -------------------------------------------------------");
      New_Line;
      ...
      --  Deviation analysis
      Put_Line ("[CALIBRATE] Deviation from flight data:");
      Put_Line ("  Heat flux : " &
                Float'Image (abs (Metrics.Stag_Heat_Flux_Wcm2 - Target_Heat_Flux)
                             / Target_Heat_Flux * 100.0) & "%");
      Put_Line ("  Decel     : " &
                Float'Image (abs (Metrics.Decel_G - Target_Decel_G)
                             / Target_Decel_G * 100.0) & "%");
      Put_Line ("  Beta      : " &
                Float'Image (abs (Metrics.Ballistic_Coeff - Target_Beta)
                             / Target_Beta * 100.0) & "%");
      New_Line;

      Put_Line ("[CALIBRATE] NOTE: Full validation requires --validate with SPARTA.");
   ...
   procedure Main_Program is
      ...
      pragma Unreferenced (Nose_Type_Str, Payload_File_Str);
