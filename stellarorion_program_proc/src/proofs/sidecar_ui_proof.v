(* SKELETON PROOF - TRANSPARENCY NOTICE
   ...
   Unit : sidecar_ui (UI state)
   Goal : Displayed metrics equal last validated solver outputs (no stale render).
   Status : Admitted (unproven axiom-free skeleton; admitted by choice,
            disclosed here rather than hidden).
   Remediation path: formalize displayed in Coq against extracted specs.
...
(* Key property this unit must guarantee: Displayed metrics equal last validated solver outputs (no stale render). *)
Lemma sidecar_ui_key_property : True.
Proof.
